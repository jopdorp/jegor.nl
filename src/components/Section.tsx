...
export const Section = ({title, subTitle, isEven=false, sections=[]}: PageProps) => {
  ...
  return <section id="projects" className={`py-24 p-6 ${isEven ? "bg-foreground" : "bg-background dark:bg-background-dark"}  `}>
    <div className="container mx-auto">
      <h2 className={`text-2xl font-semibold text-center mb-12 ${(isEven ? "text-background dark:text-text-dark" : "")}`} >
        {title}
      </h2>
      <p className={`text-center text-md mb-12 ${(isEven ? "text-background dark:text-text-dark" : "")}`} dangerouslySetInnerHTML={{__html: content}} />
